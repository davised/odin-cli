...
/* border_overhead returns the total display width consumed by borders,
	 separators, and padding — everything except column content. */
@(private = "file")
border_overhead :: proc(t: Table, num_cols: int) -> int {
...
/* distribute_extra expands columns proportionally to fill extra space.
	 Columns with max_width are capped; leftover is redistributed. */
@(private = "file")
distribute_extra :: proc(widths: []int, columns: []Column, extra: int) {
	remaining := extra
...
/* distribute_deficit shrinks columns proportionally to meet a target width.
	 Columns respect min_width; absolute minimum is 1 (enough for ellipsis). */
@(private = "file")
distribute_deficit :: proc(widths: []int, columns: []Column, deficit: int) {
	remaining := deficit
	locked := make([]bool, len(widths), context.temp_allocator)
