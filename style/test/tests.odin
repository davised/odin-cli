...
test_parse_color :: proc(t: ^testing.T) {
		...
		for test_case in test_cases {
				...
				testing.expectf(t, ok == test_case.expected_ok, "parse_color: expected ok to be %v, got %v for input '%s'",
						test_case.expected_ok, ok, test_case.input_string,)
				if ok {
...
test_st :: proc(t: ^testing.T) {
		...
		for test_case in test_cases {
				...
				testing.expectf(t, ok == test_case.expected_ok, "st: expected ok to be %v, got %v for input '%s' with style '%s'",
						test_case.expected_ok, ok, test_case.text, test_case.style_string)
				if ok {
		...
		test_cases := []struct {
		...
		}{
				{
						style.Styled_Text{
								text = "hello",
								style = style.Style{
										text_styles = {style.Text_Style.Bold},
